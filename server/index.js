...
async function main() {
  ...
  app.get(['/company/inventory/warehouses/:id', '/company/inventory/warehouses/:id/'], sendCompany);
  ...
  app.get('/api/admin/inventory-warehouses', requireAdmin, requirePerm('inventory', 'view'), async function (_req, res, next) {
    try {
      res.json({ ok: true, warehouses: await store.listWarehouses() });
    } catch (err) { next(err); }
  });
  ...
  app.get('/api/admin/inventory-warehouses/:id', requireAdmin, requirePerm('inventory', 'view'), async function (req, res, next) {
    try {
      ...
      if (!warehouse) return res.status(404).json({ ok: false, error: 'Warehouse not found.' });
      res.json({ ok: true, warehouse: warehouse });
    } catch (err) { next(err); }
  });

  app.post('/api/admin/inventory-warehouses', requireAdmin, requirePerm('inventory', 'edit'), async function (req, res, next) {
    try {
      res.json({ ok: true, warehouse: await store.createWarehouse(req.body || {}) });
    } catch (err) { next(err); }
  });
  ...
  app.delete('/api/admin/inventory-warehouses/:id', requireAdmin, requirePerm('inventory', 'edit'), async function (req, res, next) {
    try {
      ...
      if (!ok) return res.status(404).json({ ok: false, error: 'Warehouse not found.' });
      res.json({ ok: true });
    } catch (err) { next(err); }
  });
  ...
  function staffPayload(body, opts) {
  ...
  app.delete('/api/admin/staff/:id', requireAdmin, requirePerm('settings', 'edit'), async function (req, res, next) {
    try {
      ...
      if (String(req.admin.id) === String(id)) {
        return res.status(400).json({ ok: false, error: 'You cannot remove your own login.' });
      }
      const all = await store.listAdmins();
      const current = all.find(function (row) { return String(row.id) === String(id); });
