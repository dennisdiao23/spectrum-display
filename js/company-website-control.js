/**
 * Company → Website → Control: visual editor for catalog products, brands, and pictures.
 */
(function (global) {
  'use strict';
  ...
  function setModeButtons() {
    ...
    if (editor) editor.classList.toggle('hidden', S.pane === 'preview');
  }
  ...
  function renderList() {
    ...
    list.innerHTML = S.products.map(function (p) {
      var on = String(S.selected) === String(p.dbId) || (S.creating && !S.selected) ? '' : '';
      on = String(S.selected) === String(p.dbId) ? ' is-on' : '';
    ...
    if (S.creating) {
      list.insertAdjacentHTML('afterbegin',
        '<button type="button" class="wc-row is-on" data-wc-id="new">' +
  ...
  function renderProductEditor(p) {
    ...
    editor.innerHTML =
      ...
      '</form>';
    toggleTypeFields();
    setLiveLink(p && p.id ? productLiveUrl(p) : '/products');
    refreshPreview(p && p.id ? productLiveUrl(p) : '/products');
  }
  ...
  function selectProduct(id, opts) {
    ...
    setModeButtons();
    renderList();
    if (S.creating) renderProductEditor(null);
    else renderProductEditor(productById(S.selected));
  }
  ...
  async function saveProduct(e) {
    ...
    try {
      ...
      await load();
      if (data.product) selectProduct(data.product.dbId);
  ...
  function onChange(e) {
    if (e.target && e.target.id === 'wc-type') toggleTypeFields();
  }
  ...
  async function load() {
    ...
    try {
      ...
      showError(err.message || 'Could not load website control.');
    }
    S.loading = false;
  }
