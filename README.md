...
| **BAKO** | Partner | Fine Pitch 600×337.5 COB, All-in-One, rental, outdoor Spaceship, T-Pro transparent |
...
| `account.html` | Customer Sign in / My Account (saved projects, custom panels, profile). No orders and no Dealer Portal link. |
| `portal.html` | Dealer Portal (`/portal`). Separate login Spectrum creates after approval. Dealer book, quote drafts, order drafts, projects, panels, and company info. |
...
4. **Inventory** tab: on-hand qty in **panels** per pitch, or **each** for control. Receive / sell / damage / count with a note. Dealer net is on the item. Dealer Portal shows net + stock only when dealer net is above zero, never factory cost.
5. **Website → Applications**: dealer form submissions. Approve as Authorized converts the CRM lead to a Company customer and leaves the website Sign in unchanged. Then **Create portal login** (email, name, password). They sign in at `/portal`. List columns: Company, Applicant, Tax ID, Submitted, Status.
